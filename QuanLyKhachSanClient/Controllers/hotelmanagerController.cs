...
namespace QuanLyKhachSanClient.Controllers
{
    public class hotelmanagerController : Controller
    {
        ...
        {
            HttpClient client = _factory.CreateClient();//HttpClient được sử dụng để tạo và gửi các yêu cầu HTTP đến một máy chủ và nhận phản hồi từ nó.
            var response = new HttpResponseMessage();//đại diện cho phản hồi HTTP từ một máy chủ
            ...
            var result = await response.Content.ReadFromJsonAsync<List<Chitietphong>>();//Dòng này sử dụng phương thức ReadFromJsonAsync để đọc nội dung
                                                                                        //của phản hồi từ server và chuyển đổi nó thành một danh sách các đối tượng Chitietphong. 

            var phongs = new List<Chitietphong>();//tạo danh sách các đối tượng chi tiết phòng
            foreach (var item in result) //Dòng này bắt đầu một vòng lặp foreach để lặp qua từng đối tượng trong danh sách result
                                         //(các đối tượng Chitietphong đã được chuyển đổi).
            {
                var phong = new Chitietphong() // Dòng này tạo một đối tượng Chitietphong mới và gán giá trị từ các thuộc tính của đối tượng Chitietphong trong danh sách result.
                {
                    Id = item.Id,
                ...
                phongs.Add(phong);//Dòng này thêm đối tượng Chitietphong vào danh sách phongs.
            }
            ViewBag.Room = phongs;
        ...
        public async Task<IActionResult> Lichsudatphong(int Id)
        {
            ...
            if (result != null)
            {
                var phongs = new List<dynamic>(); //tạo dsanh sách các đối tượng có kiểu dy
