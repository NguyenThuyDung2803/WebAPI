...
namespace QuanLyKhachSanClient.Controllers
{
    public class adminController : Controller
    {
        ...
        {
            HttpClient client = _factory.CreateClient();
            var response = await client.GetAsync(BASE_URL + $"/api/admin/GetMonKhaiVi/");
        ...
        public async Task<IActionResult> Room_Add(Img_Chitietphong room, IFormFile imgFile)
        {
            using (var httpClient = _factory.CreateClient())
            {
                using (var content = new MultipartFormDataContent())
                {
                    ...
                    content.Add(new StringContent(room.TinhTrang.ToString()), "TinhTrang");
                    ...
                    var response = await httpClient.PostAsync(BASE_URL + "/api/Admin/Create_Room", content);
                    if (response.IsSuccessStatusCode)
                    {
                    ...
                    else
                    {
                        ...
                        {
                            ViewData["error"] = "Lỗi thêm phòng mới.";
                        }
        ...
        public async Task<IActionResult> Room_Put(Img_Chitietphong room, IFormFile imgFile)
        {
            using (var httpClient = new HttpClient())
            {
                using (var content = new MultipartFormDataContent())
                {
                    ...
                    content.Add(new StringContent(room.TinhTrang.ToString()), "TinhTrang");
                    content.Add(new StringContent(room.DienTich.ToString()), "DienTich");
                    content.Add(new StringContent(room.TamNhin.ToString()), "TamNhin");
                    content.Add(new StringContent(room.Mota.ToString()), "Mota");
                    ...
                    var response = await httpClient.PutAsync(BASE_URL + "/api/Admin/Room_Update", content);
                    if (response.IsSuccessStatusCode)
                    {
                        return RedirectToAction("Room_Index");
                    }
                    ...
                    {
                        ViewData["error"] = "Lỗi cập nhật phòng mới.";
                        return View("Room_Create");
                    }
                }
            }
        ...
        public async Task<IActionResult> Room_Delete_Conf(int id)
        {
            using (var client = _factory.CreateClient())
            {
                ...
                ViewBag.Message = "Xóa phòng thành công!";
                return RedirectToAction("Room_Index");
            }
        }
        ...
        public async Task<IActionResult> Restaurant_MonChinh_Add(MonchinhModel model, IFormFile imgFile)
        {
            using (var httpClient = _factory.CreateClient())
            {
                using (var content = new MultipartFormDataContent())
                {
                    ...
                    if (imgFile != null)
                    {
                        using (var stream = new MemoryStream())//MemoryStream là một luồng bộ nhớ để lưu trữ dữ liệu trong bộ nhớ ,. Đối tượng stream này được sử dụng để đọc và ghi dữ liệu của tệp tin hình ảnh.
                        {
                            await imgFile.CopyToAsync(stream);//Dòng này sao chép dữ liệu từ imgFile (tệp tin hình ảnh) vào stream
                            stream.Seek(0, SeekOrigin.Begin);
                            byte[] imageData = stream.ToArray();// Dòng này chuyển đổi nội dung của stream thành một mảng byte(byte[]).Dữ liệu hình ảnh được lưu trong mảng byte này.
                            content.Add(new ByteArrayContent(imageData), "Img", imgFile.FileName);//Dòng này tạo ra một đối tượng ByteArrayContent từ imageData và thêm nó vào content
                        }
                    }
