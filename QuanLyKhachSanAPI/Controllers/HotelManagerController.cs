...
namespace QuanLyKhachSanAPI.Controllers
{
    ...
    public class HotelManagerController : Controller
    {
        ...
        {
            var result = await ctx.Quanlytaikhoans.FirstOrDefaultAsync(t => (t.Email == email || t.Sdt == email) && t.PassWord == password);
        ...
        [HttpPut("suataikhoan/{idkhachhang}/{ten}/{sdt}/{email}/{cmnd}/{pass}")] //cập nhập (sửa)
        public async Task<IActionResult> SuaTaiKhoan(int  idkhachhang, string ten, string sdt, string email, string cmnd, string pass)
        {
            var cn = await ctx.Quanlytaikhoans
                ...
                .OrderBy(em => em.Id != idkhachhang) //hiển thị người muốn sửa lên đầu tiên 
                .ToListAsync();
        ...
        public IActionResult Lichsuphong(int Id)
        {
            ...
            var rooms = (from a in ctx.Phieudatphongs
                         ...
                         where a.IdKh == Id
                         group new { cate3.TenDichVu, cate3.DonGia } by new // một biểu thức đại diện cho trường dữ liệu sẽ được sử dụng để nhóm các mục.
                         {
                             a.MaDp,
                             b.TenPhong,
                             b.GiaPhong,
                             a.NgayDen,
                             a.NgayDi, //à một biểu thức đại diện cho các trường dữ liệu sẽ được trả về trong kết quả gom nhóm.
                             a.NgayTt,
                             a.PhuongThucThanhToan,
                             b.Img
                         } into g //kết quả thu đc sẽ gán cho biến g
                         select new // được sử dụng để tạo ra một đối tượng mới từ các trường dữ liệu được chọn từ khối group by trước đó.
                         {
                             g.Key.MaDp, //Trường này lấy giá trị của trường MaDp từ khối group by (khóa gom nhóm).
                             g.Key.TenPhong, 
                             g.Key.NgayDen,
                             g.Key.NgayDi,// g.Key đại diện cho khóa (key) gom nhóm, được sử dụng để nhóm các bản ghi dữ liệu vào các nhóm riêng biệt dựa trên các trường đã được chỉ định trong khối group by.
                             g.Key.NgayTt,
                             g.Key.PhuongThucThanhToan,
                             g.Key.Img,
                             TienPhong = (g.Key.NgayDi - g.Key.NgayDen).TotalDays * g.Key.GiaPhong,
                             DichVu = g.Select(x => new // để tạo ra danh sách các đối tượng mới. Mỗi đối tượng trong danh sách này sẽ có các trường TenDichVu và DonGia,
                                                        // được chọn từ kết quả gom nhóm ban đầu.
                             {
