...
namespace QuanLyKhachSanAPI.Controllers
{
    ...
    public class AdminController : ControllerBase
    {
        ...
        #region room
        public static string url_anh = "C:\\Users\\nguye\\source\\repos\\QuanLyKhachSanAPI\\QuanLyKhachSanClient\\wwwroot\\img\\new_pic";
        ...
        public async Task<IActionResult> PostWithImageAsync([FromForm] Img_Chitietphong model)
        {
            ...
            else
            {
                ...
                { 
                    var fileName = Path.GetFileName(model.Image.FileName); //lấy tên tệp hình ảnh 
                    var path = Path.Combine(url_anh, fileName); //tạo đường dẫn đầy đủ đc tải lên

                    using (var stream = new FileStream(path, FileMode.Create)) //ghi hình ảnh vào đường dẫn đã chỉ định 
                    {
                        await model.Image.CopyToAsync(stream); //sao chép dữ liệu từ tệp hình ảnh trong model.Image vào luồng dữ liệu (stream)
                    }
                    addroom.Img = "/img/" + fileName;
                }
                else
                {
                    addroom.Img = "";
        ...
        [HttpGet("GetRoom/{Id}")]
        public async Task<ActionResult<Chitietphong>> GetRoom(int Id)
        {
            var result = await ctx.Chitietphongs.SingleOrDefaultAsync(ee => ee.Id == Id);
            if (result == null)
            {
        ...
        public async Task<bool> UpdateRoom([FromForm] Img_Chitietphong model)
        {
            ...
            if (model.Image != null)
            {
                ...
                var path = Path.Combine(url_anh, fileName);
                using (var stream = new FileStream(path, FileMode.Create))
                {
                    await model.Image.CopyToAsync(stream);
                }
                findP.Img = "/img/" + fileName;
            }
            else
            {
                findP.Img = "";
        ...
        public async Task<IActionResult> Create_MonKhaiVi([FromForm] MonkhaiviModel model)
        {
            ...
            else
            {
                ...
                if (model.Img.Length > 0)
                {
                    ...
                    add.Img = "~/img/" + model.Img.FileName;
                }
                else
                {
                    add.Img = "";
        ...
        public async Task<IActionResult> DeleteMonKhaiVi(int id)
        {
